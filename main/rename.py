...
@Client.on_message(filters.private & filters.command(["rename", "r"]) & filters.user(ADMIN))             
...
@Client.on_message(filters.private & filters.command(["media", "m"]) & filters.user(ADMIN))             
async def rename_file(bot, msg):
    reply = msg.reply_to_message
    if len(msg.command) < 2 or not reply:
       return await msg.reply_text("Please Reply To An File or video or audio With filename + .extension eg:-(`.mkv` or `.mp4` or `.zip`)")
